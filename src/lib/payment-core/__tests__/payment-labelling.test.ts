...
describe('payment-labelling', () => {
  ...
  it('keeps each product on its own wallet and UTXO set', () => {
    expect(() => assertUniqueIncomeStreams()).not.toThrow();
    expect(incomeStreamFor('katoa').walletId).toBe('katoa');
    expect(incomeStreamFor('katoa').utxoSet).toBe('utxo:katoa');
    expect(incomeStreamFor('motopass').walletId).not.toBe(incomeStreamFor('katoa').walletId);
    expect(incomeStreamFor('motopass').utxoSet).not.toBe(incomeStreamFor('katoa').utxoSet);
    expect(() => incomeStreamFor('hq')).toThrow(/does not receive/);
  });
