...
interface DonateQRModalProps {
  ...
  onClose: () => void;
  address: string;
  /** Optional Lightning invoice / LNURL for wallet deep links */
  lightningUri?: string;
  recipientLabel?: string;
}

export function DonateQRModal({
  isOpen,
  onClose,
  address,
  lightningUri,
  ...
  const shareAddress = useCallback(async () => {
    ...
    try {
      await navigator.share({
        title: recipientLabel ? `Donate to ${recipientLabel}` : 'Donate with Bitcoin',
        text: tipSats ? `${tipSats.toLocaleString()} sats → ${address}` : address,
      });
    } catch {
      /* user cancelled or share failed */
    }
  }, [address, canShare, recipientLabel, tipSats]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="donate-qr-title"
    >
      <button
        type="button"
      ...
      <div className="relative z-[90] w-full sm:max-w-md max-h-[92dvh] sm:max-h-[90vh] overflow-y-auto overscroll-contain animate-sheet-up sm:animate-scale-in">
        <div className="relative bg-gradient-to-b from-charcoal-900 via-charcoal-950 to-charcoal-950 border border-white/10 sm:rounded-2xl rounded-t-[1.75rem] shadow-[0_-8px_60px_rgba(247,147,26,0.2)] sm:shadow-[0_20px_80px_rgba(0,0,0,0.5)] pb-safe">
          ...
          <div className="px-5 sm:px-8 pt-6 sm:pt-8 pb-6">
            <div className="text-center mb-5 pr-10 sm:pr-0">
              ...
              <p className="text-gray-400 text-sm">
                {recipientLabel ? recipientLabel : t('donate.scanSubtitle')}
              </p>
            </div>

            <TrustProofStrip compact className="mb-5" />

            <TipAmountPicker value={tipSats} onChange={setTipSats} className="mb-5" />

            <div className="relative mx-auto w-full max-w-[min(100%,280px)] sm:max-w-[300px] mb-6">
              <div
                className="absolute -inset-1 rounded-[1.35rem] bg-gradient-to-br from-bitcoin-orange-500/40 via-amber-500/20 to-neon-cyan-500/30 blur-sm"
                aria-hidden
              />
            ...
            <WalletDeepLinks paymentUri={paymentUri || address} className="mb-5" />

            <div className={`grid gap-3 ${canShare ? 'grid-cols-2' : 'grid-cols-1'}`}>
              <Button variant="bitcoin" size="lg" onClick={copyAddress} className="w-full gap-2 min-h-[48px]">
                {copied ? <Check size={18} /> : <Copy size={18} />}
                {copied ? t('donate.copied') : t('donate.copyAddress')}
              </Button>
              {canShare && (
                <Button variant="secondary" size="lg" onClick={shareAddress} className="w-full gap-2 min-h-[48px]">
                  <Share2 size={18} />
                  {t('donate.share')}
                </Button>
              )}
            ...
            <p className="mt-4 text-center text-[11px] text-gray-500 leading-relaxed">{t('donate.thanks')}</p>
            <p className="mt-2 text-center text-[10px] text-gray-600 leading-relaxed">{t('donate.safeHarbour')}</p>
          </div>
        </div>
      </div>
    </div>
